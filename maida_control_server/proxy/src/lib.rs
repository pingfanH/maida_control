...
async fn handle_proxy_request(mut req: Request<Body>) -> Result<Response<Body>> {
    if Method::CONNECT == req.method() {
        ...
        if host_with_port == "tgk-wcaime.wahlap.com:443" {
            tokio::spawn(async move {
                match hyper::upgrade::on(&mut req).await {
                    Ok(mut client_stream) => {
                        if let Ok(mut upstream) = TcpStream::connect(&host_with_port).await {
                            let _ = copy_bidirectional(&mut client_stream, &mut upstream).await;
                        }
                    }
                    Err(e) => log::error!("[CONNECT] 升级连接失败: {}", e),
                }
            });
        ...
        tokio::spawn(async move {
            match hyper::upgrade::on(&mut req).await {
                Ok(mut client_stream) => {
                    log::info!("[CONNECT] 隧道请求建立，目标: {}", host_with_port);
...
<body><h1>代理错误</h1><p>{}</p></body>
            ...
            {}",
                                ...
                                match res {
                                    Ok((login,user_key))=>{
                                        let front_base = env::var("FRONT_BASE_URL")
                                            .unwrap_or_else(|_| "https://192.168.10.4:5173".to_string());
                                        let redirect_url = format!(
                                            "{}/home?user_id={}&open_game_id={}&session_id={}&user_play_flag={}&new_user_id_flag={}&open_game_id_flag={}&open_user_id={}",
                                            front_base.trim_end_matches('/'),
                                            login.user_id,
                                            login.open_game_id,
                                            login.session_id,
    ...
    {}",html_body.len(),html_body);
        ...
        log::info!("[HTTP] forward {} {}", req.method(), target_url);
